...
struct FBGStructureInfo
{
	...
	FBGStructureInfo(UStaticMesh* NewWallStaticMesh, UMaterialInstance* NewWallMaskedMaterialInstance, UStaticMesh* NewCornerStaticMesh, UMaterialInstance* NewCornerMaskedMaterialInstance, UStaticMesh* NewBaseStaticMesh, UMaterialInstance* NewBaseMaterialInstance, FTransform const NewTransform)
		: WallStaticMesh(NewWallStaticMesh), WallMaskedMaterialInstance(NewWallMaskedMaterialInstance), CornerStaticMesh(NewCornerStaticMesh), CornerMaskedMaterialInstance(NewCornerMaskedMaterialInstance), BaseStaticMesh(NewBaseStaticMesh), BaseMaterialInstance(NewBaseMaterialInstance), Transform(NewTransform)
	{
	}
};
...
struct FBGPlayerInfo
{
	...
	FBGPlayerInfo(): PlayerColor(), bGameMasterPermissions(0), bReady(0)
	{
	}

	explicit FBGPlayerInfo(FText const& InName, int const& InPlayerID)
		: PlayerName(InName), PlayerID(InPlayerID), PlayerColor(), bGameMasterPermissions(0), bReady(0)
	{
	}
...
struct FBGCharacterModelSaveInfo
{
	...
	explicit FBGCharacterModelSaveInfo(int const InCharacterID, FName const InCharacterName, FVector const InCharacterLocation, FName const InCharacterModelStaticMeshName, FName const InCharacterModelMaterialName)
		: CharacterID(InCharacterID), CharacterName(InCharacterName), CharacterLocation(InCharacterLocation), CharacterModelStaticMeshName(InCharacterModelStaticMeshName),
...
struct FBGWallSplineSaveInfo
{
	...
	explicit FBGWallSplineSaveInfo(int const InWallSplineID, FName const InTags, FVector const InLocation, FName const InWallStaticMeshName, FName const InWallMaskedMaterialInstanceName, FName const InCornerStaticMeshName, FName const InCornerMaskedMaterialInstanceName, FName const InBaseStaticMeshName, FName const InBaseMaterialInstanceName)
